...
func main() {
	...
	pin := flag.Int("pin", 18, "RPIO relay pin")
	pumpOnCronIntervals = *flag.String("pcron", "0 0,6,12,18", "crond intervals")
	pumpMaxPowerOnDuration = *flag.Duration("cycle", 15*60*time.Second, "poweron cycle")
	httpPort := flag.String("http", ":8111", "HTTP listen addr")
	flag.Parse()
	...
	pump, perr := newPump(*pin, *httpPort)
